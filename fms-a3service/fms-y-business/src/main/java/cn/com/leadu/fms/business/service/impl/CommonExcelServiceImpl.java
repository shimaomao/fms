...
public class CommonExcelServiceImpl implements CommonExcelService {
                    ...
                    for(int columnNum = 0 ; columnNum <= row.getLastCellNum() ; columnNum ++){
                        ...
                        if(cell != null) {
                            if(cell.getCellType() == Cell.CELL_TYPE_NUMERIC && HSSFDateUtil.isCellDateFormatted(cell)){
                                ...
                                value = DateUtils.dateToStr(date,DateUtils.formatStr_yyyyMMddHHmmss);
                            }else{
        ...
        if(ArrayUtils.isNotNullAndLengthNotZero(datas)){
            ...
            for(Object obj:datas){
                ...
                for(int i=0;i<excelDtos.size();i++){
                    ...
                    //设置数字类型
                    if(excelDtos.get(i).getMethod().getGenericReturnType().toString().contains("BigDecimal")){
                        cellData.setCellValue(getDoubleValueFromObj(textData));
    ...
    private double getDoubleValueFromObj(XSSFRichTextString obj){
        try {
            return Double.parseDouble(obj.getString());
        }catch (Exception e){
            return 0;
        }
    }
