...
import cn.com.leadu.fms.business.common.constant.enums.activiti.ActGpsMonthlyEnums;
...
/**
 * @program: fms
 * @description: gps月结审批流
 * @author: yangyiquan
 * @create: 2018-06-05 13:46
 **/
@Component
@Slf4j
public class ActGpsMonthlyUtils {
    /**
     * 单例
     */
    private static ActGpsMonthlyUtils actGpsMonthlyUtils = null;
    ...
    public ActGpsMonthlyUtils(){
        actGpsMonthlyUtils = this;
        approvalStatusNames.put(ActGpsMonthlyEnums.GPS_MONTHLY_APPLY.getFlag(),ActGpsMonthlyEnums.APPLY_STATUS.getFlag());
        approvalStatusNames.put(ActGpsMonthlyEnums.GPS_MONTHLY_REVIEW.getFlag(),ActGpsMonthlyEnums.REVIEW_STATUS.getFlag());
        approvalStatusNames.put(ActGpsMonthlyEnums.GPS_MONTHLY_VOUCHER.getFlag(),ActGpsMonthlyEnums.VOUCHER_STATUS.getFlag());
        approvalStatusNames.put(ActGpsMonthlyEnums.GPS_MONTHLY_SETTLEMENT.getFlag(),ActGpsMonthlyEnums.SETTLEMENT_STATUS.getFlag());

        //需要设置用户的节点
        userDefKeys = new String[] {ActGpsMonthlyEnums.GPS_MONTHLY_APPLY.getFlag(),
                ActGpsMonthlyEnums.GPS_MONTHLY_REVIEW.getFlag(),
                ActGpsMonthlyEnums.GPS_MONTHLY_VOUCHER.getFlag()};
    }

    /** 
    * @Description: 启动gps月结申请
    ...
    */ 
    public static ActRuTaskVo startMonthlyAndSubmit(String serviceId, String serviceType, String serviceName){
        ...
        variables.put(ActGpsMonthlyEnums.APPLY_USER.getFlag(),applyUser);
        ActUtils.putVariables(variables, ActProcessInstanceKeyEnums.GPS_MONTHLY.getKey(),
                ActProcessInstanceKeyEnums.GPS_MONTHLY.getDesc(),serviceType,serviceName,serviceId,startUser,startUserName);
        //启动流程
        ProcessInstance processInstance = actGpsMonthlyUtils.runtimeService.startProcessInstanceByKey(ActProcessInstanceKeyEnums.GPS_MONTHLY.getKey(),serviceId,variables);
        //第一条任务是申请录入人的
        Task task = actGpsMonthlyUtils.taskService.createTaskQuery().processInstanceId(processInstance.getId()).singleResult();
        //提前获取到审批人的信息,并设置审批人
        Map<String,Object> taskVariables = setReviewUser(task);
        //自动设置为完成
        actGpsMonthlyUtils.taskService.complete(task.getId(),taskVariables);
        //返回任务详情信息
    ...
    */ 
    public static ActRuTaskVo approvalAgree(String taskId){
        ...
        Task task = actGpsMonthlyUtils.taskService.createTaskQuery().taskId(taskId).singleResult();
        if(task == null)
    ...
    */ 
    private static ActRuTaskVo approval(Integer status,String taskId,Map<String,Object> variables){
        ...
        actGpsMonthlyUtils.taskService.complete(taskId,variables);
    ...
    */ 
    private static Map<String,Object> setUser(Task task){
        if(task.getTaskDefinitionKey().equals(ActGpsMonthlyEnums.GPS_MONTHLY_APPLY.getFlag())){
            //申请节点设置复核人
            return setReviewUser(task);
        }else if(task.getTaskDefinitionKey().equals(ActGpsMonthlyEnums.GPS_MONTHLY_REVIEW.getFlag())){
            //复核节点设置财务制单人
            return setVoucherUser(task);
        }else if(task.getTaskDefinitionKey().equals(ActGpsMonthlyEnums.GPS_MONTHLY_VOUCHER.getFlag())){
            //制单节点设置付款人
    ...
    */ 
    private static Map<String,Object> setReviewUser(Task task){
        ...
        //如果申请审批人为空，则进行放入
        if(StringUtils.isTrimBlank(taskVariables.get(ActGpsMonthlyEnums.REVIEW_USER.getFlag()))){
            String approvalUserId = formPropertyMap.get(ActGpsMonthlyEnums.REVIEW_USER_ID.getFlag());
            String approvalUserUnit = formPropertyMap.get(ActGpsMonthlyEnums.REVIEW_USER_UNIT.getFlag());
            ...
            results.put(ActGpsMonthlyEnums.REVIEW_USER.getFlag(),approvalUser);
        }
        return results;
    }
    ...
    */ 
    private static Map<String,Object> setVoucherUser(Task task){
        ...
        Map<String,Object> results = new HashMap<>();
        //如果审批复核人为空，则进行放入
        if(StringUtils.isTrimBlank(taskVariables.get(ActGpsMonthlyEnums.VOUCHER_USER.getFlag()))){
            String reviewUserUnit = formPropertyMap.get(ActGpsMonthlyEnums.VOUCHER_USER_UNIT.getFlag());
            String reviewUserId = formPropertyMap.get(ActGpsMonthlyEnums.VOUCHER_USER_ID.getFlag());
            ...
            results.put(ActGpsMonthlyEnums.VOUCHER_USER.getFlag(),reviewUser);
        }
        return results;
    }
    ...
    */ 
    private static Map<String,Object> setSettlementUser(Task task){
        ...
        Map<String,Object> results = new HashMap<>();
        //如果审批复核人为空，则进行放入
        if(StringUtils.isTrimBlank(taskVariables.get(ActGpsMonthlyEnums.SETTLEMENT_USER.getFlag()))){
            String reviewUserUnit = formPropertyMap.get(ActGpsMonthlyEnums.SETTLEMENT_USER_UNIT.getFlag());
            String reviewUserId = formPropertyMap.get(ActGpsMonthlyEnums.SETTLEMENT_USER_ID.getFlag());
            Object reviewUser = ActUtils.getNextUser(reviewUserId,reviewUserUnit);
            LogUtils.infoLine(log,"流程实例: " + task.getProcessInstanceId() + "的审批付款人是: " + JSON.toJSONString(reviewUser));
            results.put(ActGpsMonthlyEnums.SETTLEMENT_USER.getFlag(),reviewUser);
            results.put(ActGpsMonthlyEnums.NEXT_USER.getFlag(),reviewUser);
        }else{
            results.put(ActGpsMonthlyEnums.NEXT_USER.getFlag(),taskVariables.get(ActGpsMonthlyEnums.SETTLEMENT_USER.getFlag()));
        }
        return results;
    }
    ...
    */ 
    private static Map<String,Object> getTaskVariables(String executionId){
        //为了防止是退回后,再次进行提交,所以要进行查询流程参数,判断每个流程的具体审批人是否已经被设置过
        Map<String,Object> taskVariables = actGpsMonthlyUtils.runtimeService.getVariables(executionId);
