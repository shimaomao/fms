...
app.controller('sys_user_modify_controller', ['$scope', '$http','$modal','toaster','$location','$compile',function ($scope, $http, $modal, toaster,$location,$compile) {
        ...
        if(!$scope.form.$invalid && checkRole()) {
            ...
            $http.put('sys_user/modifySysUser',$scope.sysUser).success(function (data) {
                if (data.code == Response.successCode){
                    $location.path("app/system_sys_user_list").search({type:'modify'});
                }
        ...
        $scope.dataTableProperties= {
            ...
            dataTableColumn: [
                ...
                {title:'操作', "render":function(data, type,row){
                    var html = '<a style="color:#2dc9ff" ng-click="alt('+'\''+data+'\''+')">删除</a>';
                    return html;},
