/**
 * Created by ningyangyang on 2018/8/6.
 */
app.controller('orig_file_borrow_back_send_examine_controller', ['$scope', '$http','$modal','$compile','$location', 'toaster',function ($scope, $http,$modal,$compile,$location,toaster) {
    ...
    $scope.submitOrigFileBorrow = function () {
        if (!$scope.form.$invalid){
            $scope.submit = true;
            $http.post('orig_file_detail/borrowBackTaskConfirm',$scope.borrowTaskVo).success(function (data) {
                if(data.code == Response.successCode){
                    //资管审核通过地址跳转地址
                    $location.path("app/home").search({'type':'homeToastInfo', 'msg':'审核确认成功'});
                }else {
                    modalAlert($modal, data.message);
                }
                $scope.submit = false;
