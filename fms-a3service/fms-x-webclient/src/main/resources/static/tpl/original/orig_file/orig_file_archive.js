app.controller('orig_file_archive_controller', ['$scope', '$http','$modal','$compile','$location','toaster','setData', function ($scope, $http,$modal,$compile,$location,toaster,setData) {
    //查询参数
    ...
    $scope.dataTableProperties= {
        dataTableAjax : {
            url : 'orig_file/findOrigFileListByPage',
            type:"GET",
   ...
   /* $scope.resetContRepaySked = function(){
    ...
    $scope.origFileUpload = function () {
        ...
        //     modalAlert($modal,'已归档不可操作');
            ...
            $location.path('/app/orig_file_upload').search({"bizCode": rows[0].bizCode, 'bizCodeType':rows[0].bizCodeType,
                'origFileType' : rows[0].origFileType,'origFileObj':rows[0],"uploadSourceFlag":2});
        }
    };
    ...
    $scope.fileOperation = function () {
        ...
        // var bizCodes =  $scope.dataTable.getRowsIds('bizCode');
        if(bizCodes.length < 1){
            modalAlert($modal,'请选择需要操作的数据');
        }else if(bizCodes[0].origFileStatus == '1' || bizCodes[0].origFileStatus == '2' ){
            modalAlert($modal,'该明细已归档，不可操作');
        }else {
            $location.path('/app/orig_file_sort').search({
                "bizCode": bizCodes[0].bizCode
                , 'bizCodeType':bizCodes[0].bizCodeType
    ...
    $scope.fileDetail = function () {
        ...
        if(bizCodes.length < 1){
            modalAlert($modal,'请选择需要操作的数据');
        // }else if(bizCodes[0].origFileStatus == 0){
