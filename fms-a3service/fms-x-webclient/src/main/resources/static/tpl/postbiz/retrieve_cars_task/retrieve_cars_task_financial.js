 ...
 *  收车财务审核
 */
app.controller('retrieve_cars_task_financial_controller',['$scope', '$http', '$modal', 'toaster','$compile','$location','$timeout',function($scope, $http, $modal, toaster,$compile,$location,$timeout){
    ...
    $scope.selectBankInfo = function () {
        ...
        rtn.result.then(function (data) {
            if(data != null) {
                $scope.retrieveCarsTask.payAccBranch = data.accBranchBank;
                $scope.retrieveCarsTask.payAccBank = data.accBankName;
                $scope.retrieveCarsTask.payAccountName = data.accountName;
                $scope.retrieveCarsTask.payAccountNo = data.accountNo;
            }
        },function(){
        });
    }
    ...
    $scope.save = function () {
        if(!$scope.form.$invalid) {
            ...
            $http.post('retrieve_cars_task/retrieveCarsTaskFinancial', $scope.retrieveCarsTask).success(function (data) {
                if (data.code == Response.successCode)
                    $location.path("/app/home").search({"type":"homeToastInfo",'msg':'财务审核成功'});
                else
                    modalAlert($modal,data.message);
                $scope.submit = false;
            }).error(function(data){
    ...
    $scope.sendBack = function () {
        ...
        $scope.retrieveCarsTask.taskId = $scope.taskId;
        $scope.retrieveCarsTask.retrieveCarTaskNo = $scope.retrieveCarTaskNo;
        $http.post('retrieve_cars_task/retrieveCarsTaskFinanceSendBack',$scope.retrieveCarsTask).success(function (data) {
            if (data.code == Response.successCode)
