 ...
 * 展期申请审批
 */
app.controller('defer_task_approve_page_controller', ['$scope', '$http','$modal','toaster','$location',function ($scope, $http,$modal,toaster,$location) {
    ...
    console.log($scope.taskDefinitionKey)
    //获取展期合同信息
     ...
     * 提交展期合同信息
     */
    $scope.submitDeferTaskApprove = function () {
        ...
        if(!$scope.form.$invalid) {
            ...
            $http.put('defer_task/submitDeferTaskApprove', $scope.deferTask).success(function (data) {
                if (data.code == Response.successCode){
                    $location.path('/app/home').search({"type": 'homeToastInfo',"msg":'审批提交成功'});//我的任务页面
    ...
    $scope.goRefuse = function () {
        ...
        $scope.deferTask.deferTaskNo = $scope.deferTaskNo;
        //拒绝操作
        $http.put('defer_task/goRefuse', $scope.deferTask).success(function (data) {
            if (data.code == Response.successCode){
                $location.path('/app/home').search({"type": 'homeToastInfo',"msg":'拒绝成功'});//我的任务页面
            }
            else
                modalAlert($modal,data.message);
            $scope.submit = false;
