app.controller('basic_change_cont_audit_controller', ['$scope', '$http', '$modal', 'toaster','$compile','$location','$interval', function ($scope, $http, $modal, toaster, $compile, $location, $interval) {
    ...
    $scope.save = function () {
        if(!$scope.form.$invalid){
            ...
            $http.post('change_lessee_task/changeContApprove',$scope.changeLesseeTask).success(function (data) {
                if(data.code == Response.successCode){
                    $location.path("/app/home").search({type:'homeToastInfo', msg:'合同审核成功'});
                }else{
                    modalAlert($modal,data.message);
                }
                $scope.submit = false;
