/**
 * Created by ningyangyang on 2018/9/26.
 */
app.controller('basic_change_detail_controller', ['$scope', '$http', '$modal', 'toaster','$compile','$location','$interval', function ($scope, $http, $modal, toaster, $compile, $location, $interval) {
    //$scope.applyType = $location.search()['serviceType'];
    $scope.taskId = '111';
    $scope.applyNo = $location.search()['applyNo'];
    //流程任务key
    $scope.taskDefinitionKey = 'change_task_detail';
        ...
        $http.get('change_lessee_task/findApplyInputVoByApplyNo?applyNo='+$scope.applyNo).success(function(data){
            if(data.code == Response.successCode){
                ...
                if($scope.applyType == 1){
                    $scope.fileObj.fileType = CommonCodeUtils.basFileTypes.perApplyFile;
                }else{
                    $scope.fileObj.fileType = CommonCodeUtils.basFileTypes.compApplyFile;
                }
