 ...
 * 制单
 */
app.controller('equ_mor_detail_voucher_controller', ['$scope', '$http','$modal','toaster','$location','$compile',function ($scope, $http,$modal,toaster,$location,$compile) {
    ...
    function init(){
        $http.get('equ_mor_detail/findEquMorDetailVoList?equRelTaskNo='+$scope.equRelTaskNo).success(function (data) {
            ...
            $scope.equMorDetailVoList = data.data.equMorDetailVoList;
            $scope.equMorDetail.payAccBank = '';
            $scope.equMorDetail.payAccountName = '';
            $scope.equMorDetail.payAccountNo = '';
            $scope.equMorDetail.payEleBankNo = '';
            //参数配置
    ...
    //付款银行
    //$scope.payAccBankList = CommonCodeUtils.getCommonCodes(CommonCodeUtils.codeTypes.openingBank);
    ...
    $scope.selectBasBankInfo = function(){
        ...
        rtn.result.then(function (data) {
            if(data != null) {
                $scope.equMorDetail.payAccBank = data.accBankName;
                $scope.equMorDetail.payAccountName = data.accountName;
                $scope.equMorDetail.payAccountNo = data.accountNo;
                $scope.equMorDetail.payEleBankNo = data.eleAccountNo;
                $scope.equMorDetail.payAccBranch = data.accBranchBank;
            }
        },function(){
        });
    }
    ...
    $scope.save = function () {
        if(!$scope.form.$invalid){
            ...
            $http.put('equ_mor_detail/approvalVoucher',$scope.equMorDetail).success(function(data) {
                if (data.code == Response.successCode){
                    //modalAlert($modal, '财务制单成功');
                    $location.path("app/home").search({"type": 'homeToastInfo', "msg":'财务制单成功'});
                }else
     ...
     * 资方解压付款单打印
     */
    $scope.printEquRel = function () {
        $scope.equMorDetail.equMorDetailVoList = $scope.equMorDetailVoList;
        CommonFileUtils.downloadFilePost('equ_mor_detail/printEquRel',$scope.equMorDetail
            ,$http,$modal,$scope);
    }
