 ...
 */
app.controller('bas_file_type_modify_controller', ['$scope', '$http','$modal','toaster','$location','$compile',function ($scope, $http,$modal,toaster,$location,$compile) {
    //是否实际文件
    ...
    if ($scope.showUpdate||$scope.showCheck) {
        $http.get('bas_file_type/findBasFileTypeVoByFileTypeId?fileTypeId='+ $location.search()['fileTypeId']).success(function(data){
            ...
            if(isNotNull($scope.basFileType.chiBasFiles)){
                $scope.chiBasFiles =  $scope.basFileType.chiBasFiles
            }
            initTable();
        });
    }
        ...
        if(!$scope.form.$invalid) {
            ...
            $http.put('bas_file_type/modifyBasFileType', $scope.basFileType).success(function (data) {
                if (data.code == Response.successCode){
                    ...
                    $location.path("app/baseinfo_bas_file_type_list").search({messageType:'modifyBasFileType'});
                }
                else
