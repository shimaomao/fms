...
app.controller('rule_detail_controller', ['$scope', '$http','$modal','toaster','$compile','$location', function ($scope, $http,$modal,toaster,$compile,$location) {
    ...
    $scope.save = function () {
        ...
        if(!$scope.form.$invalid) {
            ...
            if(isContinue){
                ...
                $http.put('rule/modifyRule', $scope.rule).success(function (data) {
                    if (data.code == Response.successCode) {
                        $location.path("app/baseinfo_rule").search({messageType:'modifyRule'});
                    }
                    else
    ...
    $scope.$on('repeatFinishCallback',function(){
        for(var i in $scope.ruleTableList){
            ...
            //第一次循环完毕后 将结果优先级赋值
            if($scope.oneRepeat){
                $("#div_tab_" + i).find("input[name='priority']").val($scope.ruleTableList[i]['priority']);
            }
        }
        $scope.oneRepeat = false;
    ...
    $http.get('rule/findRuleVoByRuleId?ruleId=' + $location.search()['ruleId']).success(function (data) {
        if (data.code == Response.successCode) {
            $scope.isMessage = false;
            $scope.rule = data.data;
            $scope.rule.ruleType  = CommonCodeUtils.getCodeValueName(CommonCodeUtils.codeTypes.ruleType,$scope.rule.ruleType);
            $scope.ruleTableList = data.data.ruleDetailVos;
            $scope.oneRepeat = true;
        }
        else
            modalAlert($modal,data.message);
    }).error(function(data){
